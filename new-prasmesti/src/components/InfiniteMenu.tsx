...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        // Circular flag badge with aspect-preserving cover-fit
        ctx.save();
        ctx.beginPath();
        ...
        // Background fill so the circle reads against the dark scene
        ...
        if (img && img.width && img.height) {
          ...
          let drawH: number;
          if (imgAspect >= 1) {
            // wider than tall — cover by height
            drawH = cellSize;
            drawW = cellSize * imgAspect;
          } else {
            // taller than wide — cover by width
            drawW = cellSize;
        ...
        // Outer ring stroke
        ctx.strokeStyle = 'rgba(212, 100, 26, 0.55)';
        ctx.lineWidth = 6;
        ctx.beginPath();
  ...
  private updateProjectionMatrix(): void {
    ...
    const height = this.SPHERE_RADIUS * 1.15;
